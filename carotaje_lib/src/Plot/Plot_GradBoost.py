...
def curve_train_and_valid(n_estimators, f1_train, f1_valid, loss_train, loss_valid, args):

    print ('Plotting training and validation curve...')

    fig = make_subplots(rows=1, cols=2, subplot_titles=['$\Large\\textbf{F1_score}$', '$\Large\\textbf{Loss}$'], 
                                                                vertical_spacing=0.1)

    fig.add_trace(go.Scatter(x=n_estimators, y=f1_train, mode='lines', name='Training', marker_color='blue'),1,1)
    fig.add_trace(go.Scatter(x=n_estimators, y=f1_valid, mode='lines', name='Validation', marker_color='red'),1,1)
    fig.add_trace(go.Scatter(x=n_estimators, y=loss_train, mode='lines', showlegend=False, marker_color='blue'),1,2)
    fig.add_trace(go.Scatter(x=n_estimators, y=loss_valid, mode='lines', showlegend=False, marker_color='red'),1,2)

    fig.add_trace(go.Scatter(x=[n_estimators[f1_valid.index(min(f1_valid))]], 
                        y=[f1_valid[n_estimators[f1_valid.index(min(f1_valid))]]], marker_color='darkgreen', 
                        name='Accuracy для<br> минимального loss'),1,1)

    fig.add_trace(go.Scatter(x=[n_estimators[f1_valid.index(min(f1_valid))]], 
                        y=[f1_valid[n_estimators[f1_valid.index(min(f1_valid))]]], marker_color='black', 
                        name='Минимальный loss'),1,2)

    fig.add_shape(type="line", row=1, col=2,
            x0=n_estimators[f1_valid.index(min(f1_valid))], y0=0.3, x1=n_estimators[f1_valid.index(min(f1_valid))], y1=0.1,
            line=dict(color="black",width=2,dash="dashdot"))

    fig.add_shape(type="line", row=1, col=1,
            x0=n_estimators[f1_valid.index(min(f1_valid))], y0=0.96, x1=n_estimators[f1_valid.index(min(f1_valid))], y1=0.86,
            line=dict(color="black",width=2,dash="dashdot"))

    fig.update_layout(plot_bgcolor='white', title=dict(text='<b>Обучение и валидация алгоритма градиентный бустинг', x=0.45, font=dict(size=20)), 
    ...
    fig.update_xaxes(row=1, col=1, tickfont=dict(size=20), gridcolor='lightgrey', zerolinecolor='lightgrey', 
                                        title=dict(text='n_estimators', font=dict(size=20)))
